...
def _testpoint(ref: str, x: float, y: float, net: str, *, pad: float = 1.0,
               layer: str = "F.Cu", value: str = "TestPoint") -> str:
    return f'''
...
def _hole(ref: str, x: float, y: float, drill: float = 3.5, net: str = "") -> str:
  ...
  (footprint "MountingHole:MountingHole_3.5mm_Pad"
    ...
    (pad "1" thru_hole circle (at 0 0) (size 7 7) (drill {drill}) (layers "*.Cu" "*.Mask") {net_expr})
  )'''
...
def healthy() -> str:
    ...
    body = rect_outline()
    for i, (x, y, net) in enumerate([(10, 10, "/SCL"), (14, 10, "/SDA"),
                                     (18, 10, "/TXD"), (22, 10, "/RXD")], start=1):
