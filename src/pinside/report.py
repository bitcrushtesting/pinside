...
def _rows(board: Board) -> list[list]:
    ...
    for t in board.test_points:
        pad = t.pad
        rows.append(["test_point", t.ref, t.signal or "-", t.net, t.side,
                     t.x, t.y, t.fx, t.fy,
                     pad.max_dimension if pad else "", pad.drill if pad and pad.drill else ""])
    for h in board.mounting_holes:
        rows.append(["mounting_hole", h.ref, "", h.net, "-",
                     h.x, h.y, h.fx, h.fy, h.pad_diameter or "", h.drill or ""])
    return rows


HEADER = ["kind", "ref", "signal", "net", "side",
          "dut_x", "dut_y", "fix_x", "fix_y", "pad_mm", "drill_mm"]
...
def emit_table(board: Board, findings: list[Finding], out=sys.stdout) -> None:
    ...
    if box:
        ...
        print(f"outline       {d['width_mm']} x {d['height_mm']} mm, {closed}, "
              f"({d['min_x']}, {d['min_y']}) .. ({d['max_x']}, {d['max_y']})", file=out)
    print(f"frame         origin={board.frame.get('origin')} mirror={board.frame.get('mirror')}",
          file=out)
...
def emit_json(board: Board, findings: list[Finding], out=sys.stdout) -> None:
    ...
    payload = {
        "source": board.source,
        "frame": board.frame,
        "outline": {"closed": board.outline.closed,
                    "bbox": box.as_dict() if box else None,
                    "shapes": board.outline.shapes},
        "test_points": [{"ref": t.ref, "value": t.value, "signal": t.signal, "net": t.net,
                         "side": t.side, "x": t.x, "y": t.y, "fx": t.fx, "fy": t.fy,
                         "pad": None if not t.pad else
                         {"shape": t.pad.shape, "type": t.pad.type, "size_mm": list(t.pad.size)
                          if t.pad.size else None, "drill_mm": t.pad.drill}}
                        for t in board.test_points],
        "mounting_holes": [{"ref": h.ref, "x": h.x, "y": h.y, "fx": h.fx, "fy": h.fy,
                            "drill_mm": h.drill, "pad_dia_mm": h.pad_diameter,
                            "plated": h.plated, "net": h.net} for h in board.mounting_holes],
...
def emit_svg(board: Board, findings: list[Finding], out=sys.stdout) -> None:
    ...
    pad = 12.0
    flagged = {ref.split()[0].split("-")[0].split("+")[0]
               for f in findings if f.severity == "error" for ref in f.refs}

    print(f'<svg xmlns="http://www.w3.org/2000/svg" width="{w + 2 * pad}mm" '
          f'height="{h + 2 * pad}mm" viewBox="{-pad} {-pad} {w + 2 * pad} {h + 2 * pad}">', file=out)
    print('<rect x="%g" y="%g" width="%g" height="%g" fill="#ffffff"/>'
          % (-pad, -pad, w + 2 * pad, h + 2 * pad), file=out)
    ...
    print(f'<path d="{" ".join(path)}" fill="none" stroke="#b8860b" stroke-width="0.25"/>',
          file=out)

    for hole in board.mounting_holes:
        r = (hole.drill or 3.0) / 2
        print(f'<circle cx="{hole.fx}" cy="{hole.fy}" r="{r:g}" fill="none" stroke="#2f6fd0" '
              f'stroke-width="0.25"/>', file=out)
        print(f'<text x="{hole.fx + r + 0.6:g}" y="{hole.fy + 0.5:g}" font-size="1.8" '
              f'font-family="monospace" fill="#2f6fd0">{hole.ref}</text>', file=out)

    for t in board.test_points:
        colour = "#c62828" if t.ref in flagged else "#1b7f3b"
        print(f'<circle cx="{t.fx}" cy="{t.fy}" r="0.685" fill="none" stroke="{colour}" '
              f'stroke-width="0.2"/>', file=out)
        print(f'<circle cx="{t.fx}" cy="{t.fy}" r="0.15" fill="{colour}"/>', file=out)
        print(f'<text x="{t.fx + 1.2:g}" y="{t.fy + 0.6:g}" font-size="1.5" '
              f'font-family="monospace" fill="#222">{t.ref} {t.signal}</text>', file=out)
    print("</svg>", file=out)
